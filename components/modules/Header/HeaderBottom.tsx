...
const HeaderBottom = () => {
  ...
  const changeMobileVisible = () => {
    setMobileVisible(mobileVisible => !mobileVisible)
  }
  console.log('mobileVisible', mobileVisible)
  return (
    <div className={`${styles.header__bottom} ${darkModeClass}`}>
      <div className={`container ${styles.header__bottom__container}`}>
        ...
        <div className={styles.header__menu}>
          <div className={`${styles.header__menu__item} ${darkModeClass}`} onClick={() => setSelectedCategory({category_id: 10, category_name: 'Экстерьер'})}>
          ...
          <div className={`${styles.header__menu__open_button} ${darkModeClass}`}>
            <svg width="30" height="24" viewBox="0 0 30 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={changeMobileVisible}>
      ...
      {mobileVisible ? 
        <div className={`container ${styles.header__mobile__container } ${darkModeClass}`}>
          <div className={`${styles.header__mobile__close} ${darkModeClass}`} onClick={() => setMobileVisible(false)}>X</div>
          <Link href="/main" legacyBehavior passHref>
            <a className={styles.header__logo__link}>
              {mode === 'dark' ?
              <LogoSvgDark/>
          ...
          <div className={`${styles.header__mobile__acc} ${darkModeClass}`}>
            <ProfileDropDown/>
          </div>
          <Link href="/catalog" legacyBehavior passHref>
              <a>
          ...
          <Link href={shouldLoadContent ? '/favorites' : '/'} legacyBehavior passHref>
              <a>
                  Избранное
              </a>
            </Link>
            <Link href={shouldLoadContent ? '/cart' : '/'} legacyBehavior passHref>
              <a>
                  Корзина
              </a>
            </Link>
            <Link href="/about" passHref legacyBehavior>
              ...
              <div className={`${styles.header__mobile__mode} ${darkModeClass}`}>
                <ModeToggler/>
              </div>
